...
PLUGIN_LOG_TITLE='KGSCR'	# Log Title
...
def Start():
	...
	HTTP.Headers['User-agent'] = 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_10_1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/41.0.2227.1 Safari/537.36' \

class KGSCR(Agent.Movies):
	name = 'KGSCR'
	...
	def Log(self, message, *args):
		if Prefs['debug']:
			Log(KGSCR_LOG_TITLE + ' - ' + message, *args)
	...
	def search(self, results, media, lang, manual):
		...
		if final_dir is None
			return
		#file_studio = file_name[file_name.find("(")+1:file_name.find(")")]
		#used in if statment for studio name
